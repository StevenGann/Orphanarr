...
>
> **Round 02 vote: 5 `APPROVE (with reservations)`, 0 `REVISE`.** Unanimous, against a threshold
> of ≤1 dissenter (`team/PROCESS.md` §2). Round 01 was 2 `APPROVE`, 3 `REVISE`.
>
> Requirements: `docs/BRIEF.md` — **24 open questions there are unanswered and four of them block
> or reshape design work.** Raw round-01 responses: `team/rounds/round-01-design/`.
> Conflicts resolved and dissents recorded: **Appendix B**. Sourced facts: **Appendix A**.
>
> Ratified is not finished. Every agent approved *with reservations*, and the ~25 defects they
> raised at the round-02 vote were applied in round 03 (below). None was data-loss class; all had
> one-line remedies. The document has now been wrong about its own agent tallies twice, which is
> why §0 tells you to trust the citations over the counts.
...
Neither is in the qBittorrent wiki or the TRaSH guides in those words. Both rest on project source
plus, for the second, a passing test in `tests/verification/` — **not** on how many agents agreed.
...
| S6 | **Hardlink-first** placement, verified-copy fallback, journalled, rollbackable |
...
| Non-goal | Reason |
|---|---|
| Download clients other than qBittorrent | The brief names qBittorrent as the minimum viable requirement and the stakeholder's need is *multiple instances*, not multiple products. Deluge (labels via an optional plugin) and SABnzbd (categories are mandatory-with-a-default, so "no category" barely exists) have materially different category semantics. Designing an abstraction from zero examples produces an interface shaped like exactly one implementation. |
...
| Writing tags or rewriting archives | **Architecturally forbidden, not merely deferred.** Under hardlinking the library file and the seeding file are one inode; an in-place mutation corrupts the torrent and fails its next hash check. Proven: `tests/verification/fs_semantics_test.py#C5`. |
| `chmod`/`chown` on **hardlinked** files | Same reason — permissions live on the inode. Proven: `#C19`. Orphanarr sets modes on directories it creates and on files it **copied** (a copy is a fresh inode with no relationship to the source), never on a link. |
...
1. `client.DownloadClient` — justified by BRIEF §2.1. Shaped by what we need, not by a union of
   every client's features. Note what is *absent*: no `Pause`, `Delete`, `Recheck`, `SetLocation`,
   `SetCategory`. Those are forbidden by **I8**, so they are not in the interface.

   ```go
   type DownloadClient interface {
       ID() string
       Probe(ctx context.Context) (ClientInfo, error)   // app version, api version, auth mode
       ListTorrents(ctx context.Context) ([]Torrent, error)   // ALL torrents — see §3.3
       ListFiles(ctx context.Context, infohash string) ([]TorrentFile, error)
       AddTags(ctx context.Context, infohash string, tags []string) error   // the ONLY mutation
   }
   ```
...
| O10 | No resolved path overlaps a **categorized** torrent on **any** configured client | The cross-seed gate. §3.4 FP-1. |
...
orphan by O1.
→ Three independent defenses, because this one is unrecoverable by the user: the global path
index (**O10**), content fingerprinting (§3.5), and hardlink-first placement, which makes the
whole class survivable rather than fatal.
...
confidence-reducing evidence (FP-8), which catches the already-imported case; the settle window and
stability gate; the re-check immediately before executing; and hardlink-first placement, which
makes racing an \*arr survivable (worst case: one duplicate library entry, recoverable) rather than
destructive. Read-only \*arr queue exclusion is a v1.1 proposal (Appendix B, D9), and BRIEF §5 Q7
asks the stakeholder whether any \*arr is actually running with a blank category.
...
**`.fb2` carries a plan warning**: it is a confident ebook classification, but no server in the
brief's target set ingests it — Audiobookshelf's `SupportedEbookTypes` is
`epub pdf mobi azw3 cbr cbz`, and Readarr's is `.epub .kepub .mobi .azw3 .pdf`. Filed silently it
is invisible forever, so the plan says so.
...
Komga *"will create a Series for each subfolder of any library, whatever the depth"* and *"a Book
for each file found, placed inside the Series corresponding to the parent folder of the file."*
The folder **is** the series name — a structural fact, not a convention.
...
- Series name from the parse, stripped of trailing volume-range/year/tag runs
  ...
  number was parsed confidently, in which case zero-pad — **Komga sorts books by filename**, so
  `v01`…`v41`, never `v1`…`v41`.
...
- **Do not convert `cbr`→`cbz`.** Converting would rewrite a hardlinked, seeding file (I1).
- **The converse is worth knowing, because it is the one place a media server reaches into our
  hardlinks.** Komga's own `convertToCbz` and `repairExtensions` (both default **off**) *delete*
  and *rename* files inside the library — and under hardlinking those files are the seeding
  torrent's inode. The torrent survives (`#C3`/`#C4`: unlinking or renaming one name leaves the
...
### 5.7 Ebooks — disabled by default, blocked on a stakeholder decision
...
`libraries.ebooks.enabled` defaults to **false** and ebook-classified orphans sit in review with an
explanatory message. **We must not silently pick a server and pretend the question was answered.**
See BRIEF §5 Q2.

When enabled, the emitted tree is server-agnostic and directly usable by Audiobookshelf, Kavita,
Komga's EPUB support, and Booklore:

```
{ebooks_root}/{Author}/{Title}/{Title}.{ext}
```
...
- **255 *bytes* per path component, not characters.** 132 non-ASCII characters can exceed
  ...
  as a plan warning. **Budget 237 bytes, not 255, whenever the planned operation is a copy** —
  §6.5 appends the 18-byte `.orphanarr-partial` suffix in the destination directory, so a name
  truncated to exactly 255 fails `ENAMETOOLONG` on the copy path: the long non-ASCII title this
  rule exists to support. **The extension and the structural marker are both protected** — the episode
...
3. **It is what the ecosystem already does.** Sonarr and Radarr import by hardlink and leave the
   torrent seeding.
4. **The cost is bounded and visible.** Zero bytes when hardlinking works; a second copy when it
   doesn't — and Orphanarr says so, in the plan, in gigabytes, before it happens.

The objection is that this doesn't fully solve *"orphans pile up in the completed directory
forever."* Correct. But the brief's stated harm is *"Nobody imports them,"* not "my disk is full."
Cleanup is a separate feature with a separate risk profile and it belongs to the stakeholder to
request. BRIEF §5 Q4.
...
| Situation | Operation |
|---|---|
| Hardlink probe says the pair is linkable | **`os.Link`** — one syscall, O(1), zero bytes, torrent never notices |
...
The UI reports the outcome as *"linked 40, copied 3 (EXDEV) — 41 GB used"*, not as an
undifferentiated success.

Hardlinking is also the only operation here that is **safe by default**: `link(2)` returns `EEXIST`
rather than overwriting. Contrast `rename(2)`, which destroys an existing destination silently and
with no error (proven: `#C9` — `b'IRREPLACEABLE'` became `b'replacement'`).
...
2. **The only sound probe is a real `link(2)`**, run once per (download root, library root) pair at
   ...
   *hardlink available* / *copy only — separate mounts*.
3. **The remediation is a mount-topology fix, not a filesystem fix**, and the UI must say so:
   one `-v /data:/data`, not separate `-v` for downloads and media.
...
- **A hardlink is not a snapshot** (`#C5`). Writing through one name is visible through the other.
  → I1 forbids tag writing, archive rewriting, and in-place mutation.
- **Permissions live on the inode** (`#C19`). `chmod` on the library copy also chmods the download
  copy. → Orphanarr sets modes on directories it creates, never on linked files.
- **Ownership is inherited, and this is the #1 silent failure.** qBittorrent running as uid 1001
  with umask 077 writes `-rw------- 1001:1001`. Plex runs as 1000. The hardlink succeeds.
  ...
  → **Required preflight:** per (source, library) pair, stat a source file and evaluate whether the
  configured `media_server_uid`/`media_server_gid` could read it. If not, warn **in the plan,
  before execution**, with the actual mode and owner and the suggested fix. This turns the most
  demoralizing possible failure into a visible one.

### 6.5 Copy

Every copy:

1. Writes to `{dst}.orphanarr-partial`, **in the final directory** — so the publish step is
...
(`.orphanarr-partial` has no leading dot and is **not** a hidden file — the name exists to be
greppable and to be swept by `Reconcile()`, not to be invisible.)
...
**Verification defaults to size + successful `fsync`.** Checksumming a 60 GB remux costs minutes of
I/O for a failure mode Go's stdlib does not exhibit. `verify: checksum` (BLAKE3) is opt-in, and it
applies to copies only — checksumming a hardlink is theatre, since source and destination are one
inode. *(Dissent: D5.)*

**Free space** is checked before any plan containing a copy: `Statfs(destRoot)`, require
`available - plan_copy_bytes >= reserve` (default 1 GiB, plus a 5% margin). Refuse the **whole
plan** with the numbers. A tool that fills a user's array at 3 a.m. and then can't write its own
...
| Exists, same `(dev, ino)` as source | **already linked in.** No-op, mark done. This is what makes re-runs free and retries safe. |
| Exists, same size and matching fingerprint | no-op, `probable_duplicate` note |
...
for each step in state in_progress:
    ...
    ALWAYS, after the branch resolves:
        delete any matching .orphanarr-partial          <-- unconditional, not an elif
...
4. **Libraries.** One root per media type. On each pick, immediately show: writability, free space,
   ...
   probe** — `Hardlinks: YES` or `Hardlinks: NO — will COPY; your N pending orphans need N GB`,
   with the single-mount remediation spelled out. Validates root nesting (movies ⊄ tv, Komga's
   no-shared-path rule, no library root inside a download path).
5. **Baseline scan, dry-run.** *"Found 1,412 uncategorized completed torrents, 38.4 TB. Classified:
   601 movies, 340 TV, 88 music, 12 comics, 4 ROM sets, 367 unclassified."* **Zero files touched.**
...
refactored away. A test suite asserting I1–I11 against the fault-injecting `fsx.FS` is the
...
| **I4** | Never act on a candidate whose resolved paths overlap a **categorized** torrent on any client. Overlaps among uncategorized torrents collapse into one plan. |
...
- `max_plans_per_run` (default 25).
- Free-space preflight for copy plans; abort whole-plan, never partial.
- Global kill switch honoured at the next step boundary.
...
| `ENOSPC` mid-copy | Unlink the partial, halt the plan, no auto-rollback, Health warning. |
...
2. **`fs-semantics` CI job.** Loopback ext4 image + tmpfs + a second device to force real `EXDEV`,
   plus the unprivileged-user-namespace bind-mount test. Asserts I1–I11 against **real filesystems,
   not mocks** — mocked filesystem tests would have hidden every hazard in §6.3. If a runner cannot
   provide two filesystems the script exits 2, and **that skip must be visible, not silent. Do not
   delete this test to make CI green.**
...
**Documented compose pushes a single data mount, and says why right there:**
...
Consolidated into **`docs/BRIEF.md` §5**. The four that block or reshape design work, rather than
merely configure it:

- **Q1 — Are the download roots and the library roots on one filesystem, one mount, inside the
  container?** If not, hardlinking is impossible and every operation is a full copy with double
  disk usage. This determines whether v1 is useful at all.
- **Q2 — Which ebook and audiobook servers?** Audiobookshelf is verified and easy to target.
  Calibre is not — writing into a Calibre library is forbidden by Calibre's own manual, so
  supporting it means a watched-import-folder handoff, which is a materially different feature.
- **Q3 — Are all the qBittorrent instances on the same host and filesystem as Orphanarr?** A remote
  or seedbox instance cannot be linked from at all; that instance is out of scope for v1 or needs a
  network-copy design.
- **Q4 — "Multiple download clients": multiple qBittorrent instances, or other client products?**
  v1 is scoped as the former.
...
per subfolder and a Book per file; Audiobookshelf parses author/series/sequence/year/narrator/ASIN
...
| **D4** | Job-level staging tree vs per-file staged publish | **Per-file `.orphanarr-partial` + no-clobber publish. No job-level staging tree in v1.** Per-file atomicity already guarantees no partial *file* is ever visible; a partially populated *directory* is something media servers rescan. Simplicity tiebreak. **(Round-01 wording said "+ rename only"; §6.5 no longer publishes with `rename` and an implementer building from this summary instead of the normative section would ship the exact hazard three agents blocked on. Corrected in round 03 — when a section changes, grep everything that summarizes it.)** | *Arr Expert*: media-server scanners watch these directories, and a half-built season folder can be scanned, half-matched, and **cached**. Reopen on evidence that Plex or Jellyfin caches a bad match from a partially-populated folder — that is a concrete, testable claim. |
...
| **D9** | Read-only \*arr queue exclusion in v1 | **v1.1 proposal.** v1 ships `ignore_save_paths` globs plus a loud first-run banner. Hardlink-first makes racing an \*arr survivable — the worst case is a duplicate library entry, which is recoverable. | *Arr Expert* wanted it in v1, with source proving Sonarr polls without a category filter when its category field is blank. Reopen if the stakeholder confirms any \*arr is running with a blank category (BRIEF §5). |
...
|---|---|---|
| `tests/verification/fs_semantics_test.py` | 19 claims about `link`/`rename`/`replace`/copy/`NAME_MAX`/permissions/reflink against two real filesystems. The regression net for every assertion in §6. | 19 checked, 0 refuted |
